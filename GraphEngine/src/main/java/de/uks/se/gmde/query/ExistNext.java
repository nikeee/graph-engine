...
public class ExistNext implements GraphQuery<LtsPath>
{
	private final Predicate<Graph> p;

	public ExistNext(final Predicate<Graph> p)
	{
		if (p == null)
	...
	{
		Optional<Destination> exampleDest = testInner(state);
		return exampleDest.map(dest -> LtsPath.create(state, dest));
	}
